...
public class DisponibleDAO {
    ...
    public void crearNuevoDisponibles() {
        ...
        if (comprobarValido) {
            ...
            if (recogerSalaPase(salaBuscar.getSid(), paseBuscar.getTid()).size() == 0) {
                ...
                System.out.println("- Sesión ya reservada -");
            }
        } else {
    ...
    public void reservarEntradas() {
        ...
        if (listadoButacas != null) {
            ...
            if (comprobarDisponibilidadButaca(filaClienta, butacaClienta, salaBuscar.getSid(), paseBuscar.getTid())) {
            ...
            System.out.println("No se encontró una sesión en esa sala");
        }
    }
    ...
    public int maximoFilas(Salas sala, Pases pase) {
        iniciarOperacion();
        int maxFilas = (int) sesion.createQuery("SELECT MAX(d.fila) from Disponible d where sid=" + sala.getSid() + " AND tid=" + pase.getTid())
                .list().get(0);
        return maxFilas;
    }
    ...
    public String pedirButacaReserva(String pregunta, int fila, List<Disponible> listadoButacas) {
        ...
        do {
            ...
            if ((butacaReserva.length() < 0 && butacaReserva.length() > 3)
    ...
    public boolean comprobarDisponibilidadButaca(int fila, String butaca, int sid, int tid) {
        ...
        iniciarOperacion();
        int estado = (int) sesion.createQuery("SELECT estado FROM Disponible d WHERE sid=:param1 AND tid=:param2 AND d.fila=:param3 AND d.letra=:param4")
    ...
    public void contarEspectadores() {
        try {
            ...
            List<Object[]> listDisp = sesion.createQuery(query).list();
            //select new map( max(bodyWeight) as max, min(bodyWeight) as min, count(*) as n )from Cat cat
            // Las añadimos al listado de contabilizar
            for(Object[] obj: listDisp){
                int pid = (int)obj[0];
                int espectadores = (int)obj[1];
                System.out.println("PID: " + pid + "; ESP: " + espectadores);        
            }

            // Ordenar por espectadores
